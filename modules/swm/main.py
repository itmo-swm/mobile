...
class SWMApp(PortalApp):
        ...
        if r.status_code == 200:
            ...
            for mdate in dmessages:
                if i < msg_num:
                    ...
                    str = "[color=ff3333](%s) [b]%s:[/b] %s[/color]\n" % \
                           (mdate, message['title'], message['desription'])
                    #str = head + '\n'.join([str[i:i+120] for i in range(0, len(str), 180)])
                    #msg += head + str
                    msg += str

                    if not self.msg_txts.has_key(mdate):
                        self.msg_txts[mdate]={'text':message['title'] + " " +
                                              message['desription'],
                                              'say': 1}
                    i += 1
            self.msg_label.text = msg
            print "R: " + `self.run_audio`
            if self.run_audio:
                self.show_audio()
        else:
    ...
    def get_mod(self):
        url = '%s/aui_get?platform=%s&time=%s&battery=%f' % \
              (self.url, platform, urllib.quote(datetime.now().ctime()),
               battery.status['percentage'])
        if self.gps:
            url += "&coordinates=%s,%s" % (self.gps[0], self.gps[1])
    ...
    def show(self):
        ...
        msg_label=Label(text="Select the preferred interface type:",
                             markup=True,
                             #size_hint=(None, None),
        )
        
        vbox.add_widget(msg_label)

        for ui in sorted(self.uis, lambda x,y: cmp(y['weight'], x['weight'])):
            funcs = ui['function']
            name = reduce(lambda x, y: x + ", " + y, ui['name'])
            name = "%s (%d)" % (name, ui['weight'])
            button = Button(text = name, on_press = self.show_map)
            button.funcs = funcs
            vbox.add_widget(button)
        
        self.buttons = BoxLayout(orientation='horizontal',
                                 height='32dp',size_hint_y=None)
        self.rld_btn = Button(text="Reload",
                              on_press=lambda a: self.show())
        self.buttons.add_widget(self.rld_btn)
    ...
    def set_mod(self, funcs):
        self.mod_graph = 0
        self.mod_text = 0
        self.mod_audio = 0
        
        if 'audio_ui' in funcs:
            self.mod_audio = 1
        if 'text_ui' in funcs:
            self.mod_text = 1
        if 'graph_ui' in funcs:
            self.mod_graph = 1
            
    def show_map(self, args):
        self.set_mod(args.funcs)
...
#                             pos_hint={'x':0.20},
    ...
    def play_audio(self, text):
        ...
        sound = SoundLoader.load('message.mp3')
        if sound:
            ...
            pos = sound.get_pos()
            time.sleep(1)
            while sound.state == 'play' and pos != sound.get_pos():
                time.sleep(1)
                pos = sound.get_pos()
    ...
    def show_audio(self):
        print "SHOW AUDIO"
        if self.mod_audio:
            for msg in sorted(self.msg_txts):
                if self.msg_txts[msg]['say']:
                    p = Process(self.play_audio(self.msg_txts[msg]['text']))
                    p.start()
                    p.join()
                    self.msg_txts[msg]['say'] = 0
